...
class Delft3D:
    ...
    def x_coordinates(self):
        ...
        self._x_coordinates=self.get_variable('xzw')[range(self.space)
                ] if self._x_coordinates is None else self._x_coordinates
        return self._x_coordinates

    @property
    def y_coordinates(self):
        """Center of gravity's y-coodinates as part of `space`."""
        self._y_coordinates = self.get_variable('yzw')[range(self.space)
              ] if self._y_coordinates is None else self._y_coordinates
    ...
    def water_depth(self):
        ...
        if self._water_depth is None:
            return self.get_variable('is_sumvalsnd')[range(self.space), 2] / self.time_step
        else:
    ...
    def get_mean_hydrodynamics(self):
        ...
        current_vel = self.get_variable('is_sumvalsnd')[range(self.space), 1] / self.time_step
    ...
    def update(self, coral, stormcat=0):
        ...
        self.time_step = self.update_interval_storm if stormcat > 0 else self.update_interval
        self.reset_counters()
        self.model.update(self.time_step)

        return self.get_max_hydrodynamics() if stormcat > 0 else self.get_mean_hydrodynamics()
...
class Transect:
    ...
    def settings(self):
        ...
        msg = f'1D schematic cross-shore transect with forced hydrodynamics: '\
    ...
    def update(self, coral, stormcat=0):
        ...
        if stormcat in [0,1,2,3]:
            ...
            h = self._water_depth
            wave_vel=Hs/4*np.sqrt(9.81/h)*np.exp(-np.power((3.65/T*np.sqrt(h/9.81)),2.1))
        else : 
