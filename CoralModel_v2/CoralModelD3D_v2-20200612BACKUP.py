...
class Processes():
    ...
    def __init__(self, fme=True, tme=True, pfd=True):
        ...
        if tme:
            if not fme:
                print(
                    'WARNING: fme must be True for tme to be True.')
            self.fme = True
        else:
            self.fme = fme
        self.tme = tme
...
class Constants():
    ...
    def __init__(self, Kd0=None, thetamax=None, Cs=None, Cm=None, Cf=None,
                 ...
                 gC=None, omega0=None, kappaA=None, Xf=None, Xp=None, Xpu=None,
                 Xs=None, XsI=None, Xsu=None, u0=None, rhoc=None, sigmat=None,
                 Cd=None, rhow=None, NoLarvae=None, probSett=None, dLarv=None):
        """
...
class Temperature():
    ...
    def __init__(self, temperature):
        ...
        """
        M = DataReshape(spacetime)
        self.T = M.param2matrix(temperature, 'time')
    ...
    def coralTemperature(self, coral):
        ...
        if processes.tme:
            coral.temp = self.T + (
                (deltat * constants.ap) / (constants.k * constants.K0)) * coral.light
        else:
            coral.temp = self.T
    ...
    def PTD(self, coral, environment, year):
        ...
        def thermacc(self):
            ...
            MMM = environment.tempMMM[np.logical_and(
                ...
                environment.tempMMM.index >= year - int(constants.nn / coral.Csp))]
            if processes.tme:
                if self.firstYear:
                    NotImplemented
                else:
            ...
            self.Tlo = mmin - constants.Kvar * smin
            self.Thi = mmax + constants.Kvar * smax
...
class PopStates():
    ...
    def popstates_t(self, coral, dt=1.):
        ...
        for n in range(spacetime[1]):
            coral.pop_states[coral.K > 0, n, :] = PopStates.popstates_xy(
                    self, coral, coral.photo_rate[coral.K > 0, n], dt=dt)
            coral.p0[coral.K > 0, :] = coral.pop_states[coral.K > 0, n, :]
    
    def popstates_xy(self, coral, PS, dt=1.):
        """Population dynamics over space."""
        P = np.zeros((len(coral.K[coral.K > 0]), 4))
        # # calculations
        # growing conditions
        # > bleached pop.
        P[PS > 0., 3] = (
